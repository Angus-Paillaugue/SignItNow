<script>
    ...
    $: newPollFormValid = () => {
        return newPollName.length > 0 && newPollDescription.length > 0 && newPollImageUrl.length > 0;
    }

    async function createNewPoll(){
        if(newPollFormValid()){
            creatingNewPoll = true;
            const response = await fetch("/api/createNewPoll/", { method:"POST", body:JSON.stringify({newPollName, newPollDescription, newPollImageUrl}) });
...
<div class="max-w-screen-md mx-auto w-full flex flex-col gap-4 p-4">
    <Button class="w-full max-w-lg mx-auto py-4" on:click={() => {newPollModal = !newPollModal}}>Create a new poll</Button>
    {#if polls.length == 0}
        <h3>You haven't created a poll yet!</h3>
    {:else}
        <h3 class="mt-10">My polls</h3>
